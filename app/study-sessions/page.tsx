...
const useTimer = () => {
  ...
  useEffect(() => {
    ...
    if (savedState) {
      const { mode, timeLeft, totalTime, isRunning } = JSON.parse(savedState);
      setMode(mode);
  ...
  useEffect(() => {
    ...
    const timerState = { mode, timeLeft, totalTime, isRunning };
  ...
  return { mode, timeLeft, totalTime, isRunning, startTimer, pauseTimer, resetTimer };
};

export default function StudySessionsPage() {
  const { mode, timeLeft, totalTime, isRunning, startTimer, pauseTimer, resetTimer } = useTimer();
      ...
      <main className="flex-1 p-8 max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-10 max-w-lg mx-auto">
          ...
          <div className="text-center mb-8">
            <p className="text-sm text-black mb-2 uppercase tracking-wide">{mode} mode</p>
            <h1 className="text-7xl font-bold text-black mb-1">{formatTime(timeLeft)}</h1>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={resetTimer}
              className="px-4 py-3 bg-gray-100 text-black rounded-lg hover:bg-gray-200 transition-colors"
              title="Reset"
            >
              <RotateCcw size={18} />
            </button>
